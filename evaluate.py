...
from train_lora import LoRALayer, StableDiffusionDataset, inject_lora_to_linear
...
def calculate_ssim(original, generated, window_size=11, size_average=True):
    ...
    sigma1_sq = (
        F.conv2d(original * original, window, padding=window_size // 2, groups=channel)
        - mu1_sq
    )
    sigma2_sq = (
        F.conv2d(
            generated * generated, window, padding=window_size // 2, groups=channel
        )
        - mu2_sq
    )
    sigma12 = (
        F.conv2d(original * generated, window, padding=window_size // 2, groups=channel)
        - mu1_mu2
    )
...
def save_results_as_dataset(eval_results, output_dir, save_visualizations=True):
    ...
    with open(output_dir / "summary.txt", "w") as f:
        ...
        f.write(
            f"Samples with original: {metadata['summary']['samples_with_original']}\n"
        )
        if psnr_values:
...
def create_visualization(result, save_path):
    ...
    images = [
        ...
        tensor_to_display(result["difference"])
        if result["has_original"] and result["difference"] is not None
        else None,
    ...
    plt.suptitle(
        f"Sample: {result['filename']} (emoji_id: {result['emoji_id']})", fontsize=14
    )
    plt.tight_layout()
...
def evaluate_model_full(args):
    ...
    # In evaluate_model_full function, replace the LoRA loading part with:
    if args.model_path:
        print(f"Loading LoRA weights from {args.model_path}")
        checkpoint = torch.load(args.model_path, map_location=args.device)

        # Try to load using diffusers' method first
        try:
            from diffusers.loaders import LoraLoaderMixin

            # Use pipeline for easier loading
            ...
            print("Loaded LoRA weights using diffusers pipeline")
        except:
    ...
    for batch_idx, batch in enumerate(tqdm(eval_dataloader, desc="Generating")):
        ...
        mask_latent = F.interpolate(
            mask, size=latents_erased.shape[-2:], mode="bilinear"
        )
        ...
        for t in noise_scheduler.timesteps:
            timestep = torch.full(
                (latents.shape[0],), t, device=args.device, dtype=torch.long
            )
        ...
        for i in range(len(filenames)):
            ...
            if has_original[i]:
                ...
                result["ssim"] = calculate_ssim(
                    original[i].unsqueeze(0), generated[i].unsqueeze(0)
                )
...
def evaluate_baseline(args):
    ...
    for idx in tqdm(range(len(eval_dataset)), desc="Evaluating"):
        ...
        if sample["has_original"]:
            ...
            result["ssim"] = calculate_ssim(
                sample["original"].unsqueeze(0), generated.unsqueeze(0)
            )
...
def main():
    ...
    # Dataset paths
    parser.add_argument(
        "--eval_data", type=str, required=True, help="Path to evaluation dataset"
    )
    parser.add_argument(
    ...
    # Output settings
    parser.add_argument(
        "--save_visualizations", action="store_true", help="Save visualization images"
    )
    parser.add_argument(
